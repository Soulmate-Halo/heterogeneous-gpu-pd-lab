...
当前版本 **v2.22**。这一版只做缩减：以前同一份数据在首页从上到下出现好多遍，现在完整数据只在「结论与核心数据」一章给出一次，后面的章节只讲每个实验想验证什么、走的是哪条路线、得出了什么结论。没做新测试，没改任何数字。准确数值都在 `results/` 的实验详档和 `data/` 的 CSV 里。
...
| **27B-PD-01**<br/>独立 PD，服务状态 | RTX 3080 20GB | 27B Q4，C1 档（C1–C6 六档全部跑通） | 3080 裸算：Prefill pp1024 **1228.53** / pp4096 **1203.06 tok/s**；Decode tg64 **33.08 tok/s** | C1，solo 路径：TTFT **4825 毫秒**、Prefill **207.2**、Decode **36.33 tok/s** | 3080 做 Prefill、395 做 Decode：TTFT **1073 毫秒**、Prefill **1000.6**、Decode **38.75 tok/s**；KV 搬运 **68–76 毫秒** | 比 395 单机：TTFT **-77.8%**、Prefill **+382.9%（4.83×）**、Decode **+6.7%**；Prefill 达到 3080 裸算的 **82%** | 服务状态下的阶段分离能跑，Prefill 不随并发下滑，Decode 也不掉。它证明的是阶段分离，不是同一阶段一起算。 |
...
| v2.6 | 3060 跑的 IQ3 和 3080 跑的 Q4 是两份割裂的 27B 记录，怎么并到一起？ | 合并成一个 27B 实验区；发布 3080 路由 v1.1 与线上 v1.2；新增 395 上的自然语言核查（27B-DRAFT-AUDIT-01）。 | 分出 Prefill 优先和 Decode 优先两种画像；立下规矩：重复文本跑出来的高分不能代表真实文本。 |
...
### 阶段四：v2.15 → v2.22 · 没换硬件、没做新测试，只整理数据归属和改写文字
