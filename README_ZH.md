...
这个仓库研究一张算力强但显存小的 NVIDIA 显卡（RTX 3060 12GB / RTX 3080 20GB）怎样和一台大内存伙伴一起跑大模型推理。原来的矩阵是这些卡配 AMD AI Max+ 395 主机。后来本地又测了一条 RTX 3080 20GB + DGX Spark GB10 的线，不和 395 矩阵合并。两台设备可以同时算同一个模型的同一个阶段，这叫**稠密加速**。稠密加速最硬的数字：9B 上 3060 + 395 组合 Prefill 2129.69 tok/s，比更快的那张单卡 1589.00 高 34.0%，比 395 单机 970.00 高 119.6%。27B Spark 组合只证明 Prefill 快于本次 3080 单机，不宣称超过所有单机。
...
395 主机上的实验归到下面五种分工。27B-SPARK-01 是另一条 3080 + Spark 线，记成第六行，不折进 395 矩阵。
...
| 双机组合 | 与 Spark 一起算 | DGX Spark GB10 与 3080 一起算 | 确认工作档上 Prefill 快于本次 3080 单机 | 27B-SPARK-01 |

395 矩阵里只有第一种是两台设备同时算同一阶段。27B-SPARK-01 是另一对硬件；它的 Prefill 提升只对照本次 3080 单机。

## 两条硬件路线与六类实验矩阵

395 主机上的数字出自下面两条硬件线、落在六格里。那些格仍是 RTX 3060 或 RTX 3080 配同一台 AI Max+ 395。第三条本地线于 2026-09-11 测得，是 RTX 3080 20GB + DGX Spark GB10，不是 395 矩阵的第七格。六格按「模型跑起来占多少显存 vs 加速卡显存」分为轻松装、装满、装不下；395 主机每格五种目标配置：RTX 3060 单卡、RTX 3080 单卡、AI Max+ 395 单机、3060 + 395、3080 + 395。同一格内模型、量化、prompt、上下文、并发、指标必须一致；单卡装不下本身是有效结果，不能换小模型或更狠量化去凑基线。395 线不能和 Spark 线放进一张表里比高低。仓库里没有任何 RTX 3090 的实测数据，这张卡在选型阶段就被排除了。

| 对比项 | 路线一：RTX 3060 12GB | 路线二：RTX 3080 20GB + 395 | 路线三：RTX 3080 20GB + Spark |
| --- | --- | --- | --- |
...
| 这条线验证了什么 | 两台设备能一起做同一个模型的预填充，并且比在场最快的单卡还快；小卡装不下的模型，分层装能跑完而且更快 | 显存大一档以后，能跑的模型、能吃的上下文、能扛的并发一起上了台阶；服务状态下的阶段分离和远端 KV 都走通了 | 确认档上组合 Prefill 快于本次 3080 单机；Decode 与均衡服务是另外的工作点 |

下面六格仍是 395 主机矩阵。路线三是另一条硬件线。
...
本地实测，硬件线与 395 矩阵不同。模型 Qwen3.8-27B，权重量化 Q4_K_M，KV 缓存 q4_0。「最高」只表示本次搜索后确认的工作档，不宣称全局最优。组合成绩不是 3080 单机成绩。数据出自 [qwen27b-spark-3080-profiles.csv](data/qwen27b-spark-3080-profiles.csv)。详档：[qwen3.8-27b-spark-3080-profiles.zh-CN.md](results/qwen3.8-27b-spark-3080-profiles.zh-CN.md)。

| 档位 | 负载 | 组合 Prefill / Decode（tok/s） | 3080 单机 Prefill / Decode（tok/s） | 牺牲了什么 |
| --- | --- | --- | --- | --- |
