...
| 阶段 | 驱动问题 | 关键验证 | 结果 |
| --- | --- | --- | ---: |
| v1.0 | 395 单卡 prefill 仅约 970 tok/s——可否让 3060 分担部分工作？ | 异构 prefill→decode 状态交接（CUDA 到 Vulkan）可稳定完成。 | 1452.29 tok/s；decode 30.28 tok/s |
| v2.1 | v1.0 让两卡*先后*工作。能否让它们*同时*服务同一次 prefill？ | 异步微批流水让同一请求的两个切层阶段同时保持激活。 | 1865.08 tok/s |
| v2.2 | v2.1 的重叠在不同轮次间仍有波动。能否稳定？ | 细化异步重叠调度使流水更稳定。 | 1893.87 tok/s |
| v2.3 | 哪种切层比例最能平衡两台后端？ | 平衡后的切层同时把 decode 抬升到 v1.0 之上。 | 1999.51 tok/s；decode 37.16 tok/s |
| **v2.5** | 重叠窗口何时才构成稠密加速？ | 校准后的检查点填满稠密区域，达到两端单卡 prefill 之和的 83.2%。 | **2129.69 tok/s** |
