...
每行一种做法、一个实验、一个关键成绩和它的对照数。

| 做法 | 实验（编号 · 模型 · 量化 · 硬件） | 关键成绩（含对照数） | 说明了什么 |
| --- | --- | --- | --- |
| 稠密加速 | 9B-PIPE-01 · Ornith 9B · Q6_K · RTX 3060 12GB + 395 | Prefill **2129.69** tok/s（3060 单卡 1589.00，395 单机 970.00）；Decode **50.73**（43.87 / 31.27） | 组合成绩超过在场最快的单设备，多出来的是 395 加进来的算力 |
| 分层装 | 27B-LONG-01 · Qwen3.8-27B · UD-IQ3_XXS · RTX 3060 12GB + 395 | pp4096 **658.52** 对 313.28；pp65536 **319.10** 对 136.69；pp98304 **225.10** 对 900 秒超时 | 3060 装不下 27B 也能参与，长 prompt 从超时变成跑完 |
| 阶段分离 PD | 9B-PD-01 · Ornith 9B · Q6_K · RTX 3060 12GB + 395 | TTFT **3.496** 秒对 5.879 秒；Prefill **1452.29** 对 861.55；Decode **30.28** 对 30.24 | 首字更快，Decode 基本不动；两段先后跑，不是叠加算力 |
| 阶段分离 PD | 27B-PD-01 · Qwen3.8-27B · Q4_K_M · RTX 3080 20GB + 395 | TTFT **1073** 毫秒对 4825 毫秒；Prefill **1000.6** 对 207.2，只到 3080 裸算 1228.53 的 82% | 天花板是加速卡自己的裸算；PD 解决首字时间与容量 |
| 远端 KV 池 | 27B-KV-01 · Qwen3.8-27B · Q4_K_M · RTX 3080 20GB + 395 | 配置 C：Prefill 1194.4–1210.6，Decode C1 33.55 / C6 63.84；配置 D：Prefill 1077–1090，Decode C1 63.2 / C6 116.3 | 两条解码路线：C 由 395 解码，D 由 3080 解码；395 都不参与 Prefill |
| MoE 的 PD | ORNITH-PD-01 · Ornith-1.5-35B-A3B · IQ4_XS · RTX 3080 20GB + 395 | 1000 输入 Prefill 4017.46 → 3943.88；100K 2895.53 → 2793.24；395 纯 Decode 23.33 → 148.20（6.35×）；42/42 请求成功 | MoE 在 100K 上下文、六档并发下 PD 稳定，两段归属分得清 |
| MoE 融合草稿 | ORNITH-PD-02 · Ornith-1.5-35B-A3B · IQ4_XS · RTX 3080 20GB + 395 | Prefill **4173.47**、单流 Decode **114.86**、草稿接受 107/114（93.86%） | 解码收益来自草稿接受率：接受率 9.4% 时只有 3665.3 / 37.2，93.86% 时 Decode 是它的 3.09 倍 |
| 单服务切层 | FLASH-SPLIT-01 · Qwen3.8-Flash · Q4 · RTX 3080 20GB + 395 | 最好用的档 C4：Prefill 633.685、聚合 Decode 71.185、总吞吐 338.270；张量切分 0.38 / 0.62 | 一个服务同时用两台设备，C4 是这份负载上的工作档位 |
...
每个实验一张小表，这是首页唯一放完整数据的地方。数字抄自 `results/` 与 `data/`；首页和详档如果有出入，以详档和 CSV 为准。提升幅度按「组合成绩 ÷ 对照成绩 − 1」算，TTFT 写的是降了多少。
...
同一次压测跑了两份负载：1000 输入 / 128 输出，以及 100000 输入 / 128 输出，两档合计 42/42 请求成功，`route=pd`、`n_reuse=0`。短的那一档当时没有单独记录 395 的纯 Decode 速率，所以这里空着。验证了什么：MoE 的 PD 在 100K 上下文、六档并发下跑得稳，Prefill 和 Decode 各算在谁头上分得清。
...
| 版本 | 实验 | 一句结论 |
| --- | --- | --- |
| v1.0 | 9B-PD-01 | 交接能成：首字 3.496 秒对 5.879 秒、Prefill 1452.29 对 861.55、Decode 30.28 对 30.24 |
| **v1.1** | 9B-PIPE-01、27B-LONG-01、EXT-DGX-01 | 9B 稠密加速走通（Prefill 1865.08 → 1893.87 → 1999.51 → 2129.69，Decode 50.73；37.16 只属于 1999.51）；27B-LONG-01 在 4K 和 64K 把 Prefill 提高一倍以上（658.52 / 319.10 对 313.28 / 136.69），98K 从超时变成 225.10 |
| v1.2 | 27B-PD-01 | 服务状态下的 PD 能跑。Prefill 683.2 → 1000.6，随后不随并发下滑、稳定在 1000–1015；C1 TTFT 1073 毫秒对 4825 毫秒。207.2 是 v1.0 口径 395 solo 实测 |
| v1.3 | 27B-KV-01、27B-DRAFT-AUDIT-01 | 关掉检查点后 3080 服务态 Prefill 1000.6 → 1210.6（1228.53 的 98.5%），395 solo 207.2 → 307.1。配置 C：Prefill 1194.4–1210.6，Decode C1 33.55 / C6 63.84。配置 D：Prefill 1077–1090，Decode C1 63.2 / C6 116.3。自然语言 C1 Decode 12.1、接受率 17.7% |
| v1.4 | ORNITH-PD-01 | 全部请求走通（42/42）。1000 输入 Prefill 4017.46 → 3943.88，100K 为 2895.53 → 2793.24；395 纯 Decode 聚合 23.33 → 148.20 |
| v1.5 | FLASH-SPLIT-01 | 计分请求全部成功（21/21）。最好用的档是 C4：Prefill 633.685、聚合 Decode 71.185、总吞吐 338.270。C1–C6 Prefill 569.892–633.685 |
| **v1.6** | ORNITH-PD-02 | Prefill 4173.47、单流 Decode 114.86、草稿接受 107/114（93.86%）。解码跟着接受率走：9.4% 时为 3665.3 / 37.2 |
