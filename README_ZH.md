...
| **v3.0（研究方向）** | v2.5 已在 AI Max+ 395 + RTX 3060 上完成一对一稠密加速验证。能否把同一机制适配到其他各类大显存主机与小显存加速卡？ | 建立跨平台适配矩阵，验证不同主机架构与加速卡型号下的显存稠密区映射、prefill/decode 无损加速和调度稳定性。 | **计划：其他各类大显存主机 + 小显存加速卡的适配加速研究** |
| **v4.0（研究方向）** | 上一阶段计划解决不同硬件的一对一适配。下一步，一张加速卡能否同时加速 X 台大显存主机？ | 研究一对多任务调度、资源隔离、公平性、故障恢复，以及并发主机数量扩大时的性能边界。 | **计划：1 张加速卡 → X 台大显存主机** |
