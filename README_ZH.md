...
当前版本 **v2.21**。这一版把开头的架构图换成了静态图片，图的源码折在图下面。之前那张图由 GitHub 自己渲染，浏览器一开整页翻译就会把源码里的关键字一起翻掉，图的位置只剩一行报错。换成图片后，不管用什么浏览器、开不开翻译都能看到。这一版没做新测试，也没动过任何一个数字。准确数值都在 `results/` 的实验详档和 `data/` 的 CSV 里。
...
### 阶段四：v2.15 → v2.21 · 没换硬件、没做新测试，只整理数据归属和改写文字
...
| **v2.21** | 开头那张架构图在部分浏览器里显示不出来，图的位置只剩一行报错。 | 查清原因，并把图换成静态图片，源码折在图下面。 | 无新测量。 | 图源码本身合法，mermaid 11.16 解析和渲染都通过；问题出在浏览器整页翻译会改写代码块里的关键字。换成图片后不受影响。 |
