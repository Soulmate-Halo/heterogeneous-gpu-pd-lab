...
当前版本：**v2.7 — 27B 双性能路线：Prefill 最快 + 单流解码最快**。公开范围仅包含架构、实测数据、思维演进、结论与限制；不提供部署步骤、复现命令、补丁、端点或内部切层策略。
...
### 新实验 27B-C｜Prefill 加速向（Prefill 最快）：最高 **1210.6 tok/s**

- 调整异构计算分工，让 3080 更集中承担输入阶段，395 专注后续生成。
- 最佳实测 Prefill 达 **1210.6 tok/s**，适合长提示词、知识库检索和长文档等首字延迟敏感场景。

### 新实验 27B-D｜解码加速向（单流解码最快）：最高 **63.2 tok/s**

- 调整解码路径与任务调度，让加速设备更直接参与连续生成。
- 最佳实测单流解码达 **63.2 tok/s**，适合低并发对话、代码生成等即时交互场景。
