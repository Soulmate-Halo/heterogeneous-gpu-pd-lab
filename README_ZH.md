...
当前版本 **v2.26**。这一版补进了一直没入库的第二个 Ornith 实验（ORNITH-PD-02）：同一个模型、同一对硬件，在独立 PD 之上加了 DFlash 融合草稿推测解码和统一 KV 池，Prefill 站上 4000 以上、单流 Decode 站上 114。1 号实验那次压测里 1000 输入档的 Prefill 也一并补齐。原有数值一个没改。准确数值都在 `results/` 的实验详档和 `data/` 的 CSV 里。
...
| --- | --- | --- | --- | --- |
| 配置 C | 1194.4–1210.6 | 33.55 | 63.84 | +90.3% |
| 配置 D | 1077–1090 | 63.2 | 116.3 | +84.0% |

每条流 1M 上下文，聚合 Decode 单位 tok/s。两列 Prefill 都是单流口径，配置 D 从 C1 到 C6 依次为 1090 / 1081 / 1080 / 1077 / 1082 / 1082 tok/s，同一轮的聚合口径从 1079 降到 1016 tok/s。配置 D 比配置 C 低约一成，是因为它要腾出解码显存，把 ubatch 从 1024 降到 512。验证了什么：想要 Prefill 高选 C，想要 Decode 总吞吐选 D。395 只存 KV、全部计算都在 3080，这是容量和服务路线，和稠密加速分开归类。
...
- **远端 KV 归容量路线**：395 只存 KV、不参与计算的配置，验证的是容量，和稠密加速分开归类。
