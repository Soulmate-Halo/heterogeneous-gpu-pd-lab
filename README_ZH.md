...
这个仓库研究一张算力强但显存小的 NVIDIA 显卡（RTX 3060 12GB / RTX 3080 20GB）怎样和一台大内存伙伴一起跑大模型推理。三条硬件路线：RTX 3060 12GB + AMD AI Max+ 395，RTX 3080 20GB + 395，以及 RTX 3080 20GB + DGX Spark GB10。两台设备可以同时算同一个模型的同一个阶段，这叫**稠密加速**。同时具备两组单机对照的最硬数字：9B 上 3060 + 395 组合 Prefill 2129.69 tok/s，比更快的那张单卡 1589.00 高 34.0%，比 395 单机 970.00 高 119.6%。本地 27B 证据，RTX 3080 20GB + DGX Spark GB10，Qwen3.8-27B Q4_K_M：组合 Prefill **1603.20** tok/s，对照本次 3080 单机 1113.13（**+44.03%**）。只有 9B 同时有单卡和单机两组对照；Spark 没有 Spark 单机基线。+44.03% 只对本次 3080 单机，不宣称超过所有单机。

当前版本 **v1.6**，七个公开里程碑，另有 2026-09-11 的本地 Spark 实测补录（不是新版本）。不公开部署命令、代码补丁、服务地址和切层策略。
...
| 双机组合 | 27B-SPARK-01 · Qwen3.8-27B · Q4_K_M · RTX 3080 20GB + DGX Spark GB10 | 组合：3080 与 Spark 一起算同一个模型。最高 Prefill 确认档，8K 输入 / 128 输出、无投机，取系统预热后三次正式测量中位数（预热轮不计）：组合 Prefill **1603.20** 对 3080 单机 1113.13（**+44.03%**），组合 Decode 17.62 对 33.36。最高 Decode 确认档开 DFlash2：2K 输入 / 128 输出，组合 Prefill 1153.95、Decode **63.97**；8K 输入 / 128 输出，组合 1124.32 / 49.20，对照 3080 单机各自调优后的 1015.15 / 58.36。速度单位均为 tok/s | 本地 Spark 实测；Prefill 快于本次 3080 单机；不是全局最优；不和社区 NVFP4 Spark 排名 |

## 为什么稠密加速是这个仓库的重点

三点优势，一点边界。

① 只有稠密加速能让组合成绩超过在场最快的单设备。Prefill 2129.69 对 3060 单卡 1589.00（+34.0%）、对 395 单机 970.00（+119.6%）；Decode 50.73 对 43.87（+15.6%）、对 31.27（+62.2%）。多出来的部分就是 395 真实加进来的算力。第二条本地稠密组合也在本页：RTX 3080 20GB + DGX Spark GB10 跑 Qwen3.8-27B Q4_K_M。最高Prefill确认档，8K 输入 / 128 输出、无投机：组合 Prefill **1603.20** 对本次 3080 单机 1113.13（**+44.03%**）。只有 9B 有双单机基线；Spark 没有 Spark 单机基线，所以 +44.03% 不是超过所有单机。
...
④ 边界：完整的单卡 + 单机两组对照目前只有 9B Q6_K + RTX 3060 这一格（D1）。27B Spark 组合在最高Prefill确认档上有匹配的 3080 单机对照，但没有 Spark 单机基线。395 主机上 27B 与 MoE 的重叠流水对照仍在后续规划。稠密区需要相邻微批能错开（重叠窗口），排不进窗口的部分归稀疏区，只负责把模型装下。
...
395 主机上的实验归到下面五种分工。RTX 3080 20GB + DGX Spark GB10 是同一组合的三个工作档：Prefill优先无投机、Decode优先 DFlash2、均衡服务 DFlash2。两机一起算同一个模型，各行只是档位目标不同。

| 分工方式 | 加速卡做什么 | 伙伴设备 | 解决什么问题 | 实验 |
| --- | --- | --- | --- | --- |
| 稠密加速 | 算前段层，与主机同时算同一阶段 | AI Max+ 395 算后段层与中间状态 | 组合成绩超过在场最快的单设备 | 9B-PIPE-01 |
| 分层装 | 只装前段层并计算 | AI Max+ 395 装后段层，把整模跑完 | 小卡装不下也能跑，且 Prefill 更快 | 27B-LONG-01 |
| 阶段分离 PD | 做全部 Prefill | AI Max+ 395 做 Decode | 首字更快；Decode 基本不动 | 9B-PD-01、27B-PD-01、ORNITH-PD-01、ORNITH-PD-02 |
| 远端 KV 池 | Prefill；配置 D 由 3080 解码 | AI Max+ 395 远端 KV；配置 C 由 395 解码 | 容量与并发 Decode | 27B-KV-01 |
| 单服务切层 | 同一 llama-server 内张量切分 0.38 | AI Max+ 395 张量切分 0.62 | 找最好用的并发档 | FLASH-SPLIT-01 |
| Prefill优先，无投机 | 与 Spark 一起算同一个模型 | DGX Spark GB10 与 3080 一起算同一个模型 | 比本次 3080 单机更快的 Prefill：组合 **1603.20** 对 1113.13（**+44.03%**）；Decode 17.62 对 33.36 | 27B-SPARK-01 |
| Decode优先，DFlash2 | 与 Spark 一起算同一个模型 | DGX Spark GB10 与 3080 一起算同一个模型 | 抬 Decode：2K **63.97** 对 60.37；8K 49.20 对 58.36（Prefill 远低于 Prefill 确认档） | 27B-SPARK-01 |
| 均衡服务，DFlash2 | 与 Spark 一起算同一个模型 | DGX Spark GB10 与 3080 一起算同一个模型 | C1–C6 一起服务：聚合 Prefill 1086.44–1097.40，聚合 Decode 24.77–47.69；126/126 成功 | 27B-SPARK-01 |
...
| 跑过的模型 | Ornith 9B 稠密 Q6_K；Qwen3.8-27B IQ3 | Qwen3.8-27B 稠密 Q4；Ornith-1.5-35B-A3B 和 Qwen3.8-Flash 两个 MoE | Qwen3.8-27B 稠密 Q4_K_M |
...
| 这条线验证了什么 | 两台设备能一起做同一个模型的预填充，并且比在场最快的单卡还快；小卡装不下的模型，分层装能跑完而且更快 | 显存大一档以后，能跑的模型、能吃的上下文、能扛的并发一起上了台阶；服务状态下的阶段分离和远端 KV 都走通了 | Prefill优先，8K 输入 / 128 输出、无投机：组合 **1603.20** / 17.62 对 3080 单机 1113.13 / 33.36（Prefill +44.03%）。Decode优先，DFlash2：2K 1153.95 / **63.97** 对 1042.97 / 60.37；8K 1124.32 / 49.20 对 1015.15 / 58.36。均衡服务，DFlash2，8K 输入 / 256 输出，C1–C6：聚合 Prefill 1086.44–1097.40，聚合 Decode 24.77–47.69；126/126 成功。无 Spark 单机基线；无 3080 单机 C1–C6。 |
...
| **D2 稠密 · 装满**<br>Qwen3.8-27B · Q4_K_M · RTX 3080 | 显存快用满时，整模塞一张卡、阶段分离、分层稠密这三条路哪条更好？ | **已验证**：27B-PD-01 用 3080 做 Prefill、395 做 Decode，服务状态下首字时间比 395 单机降了四分之三以上，C1–C6 六档全部跑通。[详档](results/qwen3.8-27b-dual-machine-pd.zh-CN.md) | **27B-SPARK-01** 三档，3080 + Spark。Prefill优先，8K/128、无投机：组合 Prefill **1603.20** 对 3080 单机 1113.13（**+44.03%**），Decode 17.62 对 33.36。Decode优先，DFlash2：2K 1153.95 / **63.97** 对 1042.97 / 60.37；8K 1124.32 / 49.20 对 1015.15 / 58.36。均衡档，DFlash2，C1–C6。[详档](results/qwen3.8-27b-spark-3080-profiles.zh-CN.md) |
| **D3 稠密 · 装不下**<br>Qwen3.8-27B · UD-IQ3_XXS · RTX 3060 | 一张卡干不完这个活时，分层装能不能跑完，而且比 395 快？ | **已验证**：27B-LONG-01 比 395 单机快一倍以上，98K 长 prompt 从超时变成跑完。[详档](results/qwen3.8-27b-dual-machine-pd.zh-CN.md) | 本地尚未测试。 |
| **M1 MoE · 轻松装**<br>模型与量化待定 | 激活参数不大、显存也有余量时，MoE 的路由开销会不会把重叠省下来的时间吃回去？ | 列入后续规划。 | 本地尚未测试。 |
| **M2 MoE · 装满**<br>Ornith-1.5-35B-A3B · IQ4_XS · RTX 3080 | MoE 快把 3080 装满时，阶段分离稳不稳；再往上做稠密重叠还有没有收益？ | **已验证**：ORNITH-PD-01 全部请求走通，100K 上下文、1 到 6 路并发下稳定，两段归属分得清（[详档](results/ornith-1.5-35b-a3b-dual-machine-pd.zh-CN.md)）；ORNITH-PD-02 在同一对硬件上加了融合草稿与统一 KV 池，3080 Prefill 4173.47、395 单流 Decode 114.86（[详档](results/ornith-1.5-35b-a3b-fused-dflash-pd.zh-CN.md)）。 | 本地尚未测试。 |
| **M3 MoE · 装不下**<br>Qwen3.8-Flash · Q4 · RTX 3080 先导 | 模型总占用超过两张基准卡时，按层或按专家分开装，能不能同时保住能跑、吞吐和输出正确？ | FLASH-SPLIT-01 是这一格的先导记录，已验证单服务切层能跑并找到最好用的并发档；完整实验列入后续规划。[先导记录](results/qwen3.8-flash-q4-layer-split.zh-CN.md) | 本地尚未测试。 |
...
**稠密加速。** 本页同时列出 9B 与 27B 两条本地稠密路线。9B-PIPE-01（3060 + 395）有 3060 单卡与 395 单机两组对照。27B-SPARK-01（3080 + Spark，Qwen3.8-27B Q4_K_M）在最高Prefill确认档上有匹配的 3080 单机对照，没有 Spark 单机基线。最高 Prefill 只比本次 3080 单机 +44.03%。
...
| 档位 | 负载 | 3080 + Spark Prefill / Decode（tok/s） | 3080 单机 Prefill / Decode（tok/s） | 牺牲了什么 |
| --- | --- | --- | --- | --- |
| 最高Prefill确认档 | 8K 输入 / 128 输出，无投机 | **1603.20** / 17.62 | 1113.13 / 33.36 | Decode 慢于 3080 单机 |
| 最高Decode确认档 | 8K 输入 / 128 输出，DFlash2 | 1124.32 / 49.20 | 1015.15 / 58.36（自己那套 DFlash2 调参） | Prefill 远低于 Prefill 确认档 |
| 最高Decode确认档 | 2K 输入 / 128 输出，DFlash2 | 1153.95 / 63.97 | 1042.97 / 60.37（自己那套 DFlash2 调参） | 与 8K Decode 行同一档；2K 与 8K 不拼接 |
| 均衡档 | 8K 输入 / 256 输出，DFlash2，C1–C6 | 见下面两张表的 C1–C6 | 未记录 | Decode 按整批窗口计时，含他路 Prefill |

后来同档 Prefill 确认档 live 复测，8K 三次为 1601.05 / 17.41。不要写成 1700。Decode 确认档对照上的投机参数按 3080 单机自己调，不是只改一个变量。

**吞吐，RTX 3080 20GB + Spark 均衡 C1–C6**

以下速度单位为 tok/s。Prefill 来自独立预填充波次；Decode 和全程输出来自输出 256 token 的完整请求波次，两类波次按并发档汇总。具体计时定义见三档详档。

| C | 聚合 Prefill | 聚合 Decode | 全程输出 |
| --- | ---: | ---: | ---: |
...
36 个正式波次，126/126 正式请求。无单机 C1–C6 对照。验证了什么：最高Prefill确认档的组合 Prefill 比本次 3080 单机 **+44.03%**。不宣称超过所有单机。
...
| 2026-09-11 补录（仍是 v1.6） | 27B-SPARK-01 | RTX 3080 20GB + DGX Spark GB10，Qwen3.8-27B Q4_K_M。Prefill优先 8K/128 无投机：组合 1603.20 / 17.62 对 3080 单机 1113.13 / 33.36（+44.03%）。Decode优先 DFlash2：2K 1153.95 / 63.97，8K 1124.32 / 49.20。均衡 DFlash2 C1–C6 126/126。不创造新版本。 |
...
- **更快**：模型、量化、负载、指标全都一致，而且有单卡或单机对照。9B-PIPE-01、9B-PD-01、27B-LONG-01、27B-PD-01 满足这条，提升幅度都在上面的表里。27B-SPARK-01 只在最高Prefill确认档上，对本次 3080 单机的 Prefill 满足这条。
...
| **Spark 组合后续** | 3080 + Spark 上已测与待测要分开写。 | **已测：** 27B-SPARK-01 三档，对照本次 3080 单机微基准（Prefill优先无投机；Decode优先 DFlash2 的 2K 与 8K；均衡 DFlash2 C1–C6）。**尚未测试：** 同模型、同量化、同负载的 Spark 单机基线；3080 单机均衡 C1–C6；组合与对照统一投机参数；这条线上的更多模型。 |
