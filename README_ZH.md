...
**四台 DGX Spark 加两张 RTX 6000Dpro，P 阶段在 32K 单请求下达到 16698.30 tok/s。** 峰值来自 2026-09-17 调优记录，输入 **32768**、输出 **128**、并发 **C1**；双 6000D 的预填充计时为 **1.962356 秒**，P 侧前缀缓存命中增量为 **0**。这才是此前约 16K 的成绩，原先首页的 13300.06 是另一组 C8 完整输入阶段的两批均值。

| 同一峰值档，32768 输入 / 128 输出 / C1 | 纯四 Spark TP4 | 双 6000D＋四 Spark | 比值 |
| --- | ---: | ---: | ---: |
| 引擎 Prefill 计时速率（组合列只计 P） | 1921.12 | **16698.30** | **8.69×** |
| 输入至首字的完整请求速率 | 1917.41 | **11348.86** | **5.92×** |
| 首字等待（秒） | 17.089707 | **2.887339** | **缩短 83.10%** |

第一行回答算力卡把 P 阶段做到多快：输入 token ÷ 引擎 prefill 计时；P 只负责输入前段，四 Spark 还要补算尾部，因此 **8.69× 是阶段速率之比**。第二行使用同一完整首字窗口，包含交接、补算和排队，才是这档请求的 **5.92× 输入处理收益**。该条记录的传输、文本烟测通过，KV 交接失败为零；不代表完整精度评测。

### 峰值对比：六卡、纯 TP4、八卡 H20

**三类机组统一按「全批成功输入 token ÷ 完整墙钟」复算。** 这个输入吞吐包含生成时间；表中均取下述测试范围内的**正式单批峰值**，不再拿本地均值对外部峰值。

| 机组 / 引擎 | 峰值负载：输入 / 输出 / 并发 | 全程输入峰值 tok/s | 六卡 / 该峰值 | 峰值批成功 |
| --- | --- | ---: | ---: | ---: |
| 纯四 Spark TP4 / SGLang | 约 8K / 1 / C4；分块 8192 | 5037.39 | **1.62×** | 4/4 |
| **双 6000D＋四 Spark / vLLM PD** | **32768 / 512 / C12** | **8165.19** | **1.00×** | **12/12** |
| 八卡 H20-3e / SGLang | 8192 / 128 / C32 | 4918.82 | **1.66×** | 64/64 |
| 八卡 H20-3e / vLLM | 24576 / 128 / C32 | 6974.62 | **1.17×** | 64/64 |

**本轮选定测试范围内，六卡全程输入峰值比八卡 H20 / vLLM 核心矩阵峰值高 17.07%。** 这是不同负载下已测峰值的数值比较；输入长度、输出长度、并发和投机设置不同，不代表同条件硬件加速倍数，也不代表 H20 的性能上限。**16698.30 是 P 阶段峰值，不能拿它除以上表 H20 的全程输入吞吐。**

取值范围：TP4 为 2048 / 4096 / 8192 分块扫描的 12 个正式 Prefill 批次（剔除预热）；六卡为 V7 正式代码矩阵的 16 批；H20 为[社区核心矩阵](https://aik8s.run/ai-k8s/practices/deepseek-v41-flash-h20-day0/)每引擎 36 轮，剔除预热、历史长度与限速实验。TP4 峰值批实际输入为 8194＋8018＋8019＋7895，共 **32126 token**。旧表的 5033.37 是 TP4 单流两批均值，本表按你的要求使用正式单批峰值 5037.39。

[峰值 CSV](data/deepseek-v4.1-flash-six-gpu-v1-v7-peaks.csv) · [逐批数字、公式与来源证据](data/deepseek-v4.1-flash-six-gpu-v1-v7-peaks.json)。H20 原报告只列输出吞吐，此处用其[逐轮原始计时](https://aik8s.run/assets/practices/deepseek-v41-flash-h20-day0/benchmark-summary.json)复算输入吞吐：SGLang = 524288 / 106.588177；vLLM = 1572864 / 225.512473。六卡 = 393216 / 48.157599。失败请求仍占用分母时间。
...
**2026-09-14 突破：Qwen3.8-Flash-Next NVFP4，RTX 6000D + DGX Spark，8K Prefill 8157.74 tok/s，C6 聚合输出 414.90 tok/s。** 这两个成绩来自同一容量配平配置的不同负载。研究也进行了**特殊配比分层（PP2）**：8K Prefill **8696.94**，C6 聚合输出 **284.56**，说明最高 Prefill 与最高生成吞吐各有适合的配比。
...
**该双机实验参数：** vLLM nightly；MoE 权重 NVFP4 / KV fp8_e4m3；上下文 65536；并发上限 6；Prefill chunk 2048；MTP 3 + 全词表草稿；容量配平档 GMU 0.99、KV 池 198332 tokens。Prefill 为 C1、4K–32K；生成压测每请求输出 256 tokens、3 轮计分 + 1 轮预热，C1–C6 合计 **63/63 成功**。PP2 两端 KV 各 8 GiB，具体分层比例不公开。
...
一张算力强但显存小的 NVIDIA 显卡（RTX 3060 12GB / RTX 3080 20GB）配一台大内存伙伴，可以一起跑大模型推理。五条路线：RTX 3060 12GB + AMD AI Max+ 395，RTX 3080 20GB + 395，RTX 3080 20GB + DGX Spark GB10，RTX 6000D + DGX Spark GB10，以及双 RTX 6000Dpro＋四 DGX Spark 的六卡 PD。**稠密加速**指两台设备同时算同一个模型的同一个阶段。当前版本 **v1.6**，七个公开里程碑，另有 2026-09-11、2026-09-14 与 2026-09-18 的本地 Spark 实测补录（不是新版本）。不公开部署命令、代码补丁、服务地址和切层策略。
...
② 阶段分离 PD 的天花板是加速卡自己的裸算。27B-PD-01 组合 Prefill 1000.6 到 3080 llama-bench 裸算 1228.53 的 82%，关掉检查点后的 1210.6 是该上限的 98.5%。裸算和服务端到端**不是同一口径**。这两组早期 PD 单流测试中，Decode 基本不动（9B：30.28 对 30.24；27B：38.75 对 36.33）。PD 分开调度两个阶段；最新 DS41-6GPU-01 进一步测得长输入并发服务的聚合 Decode 收益，不能用早期单流结论概括所有 PD 负载。
...
下表保留四条既有路线，新增六卡路线单列。这是实验覆盖地图，不是同一套负载下的统一排名。路线一是 RTX 3060 12GB + AMD AI Max+ 395。路线二是 RTX 3080 20GB + 395。路线三是 RTX 3080 20GB + DGX Spark GB10。路线四是 RTX 6000D + DGX Spark GB10。六格按「模型跑起来占多少显存 vs 加速卡显存」分为轻松装、装满、装不下；395 主机每格五种目标配置：RTX 3060 单卡、RTX 3080 单卡、AI Max+ 395 单机、3060 + 395、3080 + 395。同一格内模型、量化、prompt、上下文、并发、指标必须一致；单卡装不下本身是有效结果，不能换小模型或更狠量化去凑基线。仓库里没有任何 RTX 3090 的实测数据，这张卡在选型阶段就被排除了。
...
**P 阶段已测峰值 16698.30 tok/s（32768 / 128 / C1）；完整计时的峰值机组对比见首页首表。** 同配置 C8 Prefill 输入吞吐：8K **7812.43 对 1720.90 tok/s（4.54×）**，32K **13300.06 对 1699.75 tok/s（7.82×）**；32K 平均首字等待由 **86.541 秒降至 11.596 秒**。100/100 正式矩阵请求通过；最新部署版 V7。完整 V1–V7、参数、H20 外部参考、正确性边界见[六卡详档](results/deepseek-v4.1-flash-six-gpu-v1-v7.zh-CN.md)。
