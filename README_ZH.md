...
当前版本：**v2.8 — RTX 3080 全计算 + 395 KV 池：Prefill 最高 1210.6、聚合 Decode 最高 173.6 tok/s，每流 1M 上下文**。公开范围仅包含架构、实测数据、思维演进、结论与限制；不提供部署步骤、复现命令、补丁、端点或内部切层策略。
...
| **v2.8** | 纠正计算归属、补 C1–C6 实测、明确 1M/流与通信代价。 | 明确 3080 承担全部 Prefill 与 Decode 计算、395 为纯 KV-only 远端存储池，首页补齐真实 C1–C6 紧凑表与 C1/C6 指标。 | Prefill 最高 **1210.6 tok/s**；C1 单流 Decode **63.2 tok/s**；C6 聚合 Decode **173.6 tok/s** |
...
本实验把加速头换为 RTX 3080，记录 27B Q4 双端异构 PD。它与 9B 主表、27B-A 的 IQ3 长 prompt 扫描严格分开。下表 v1.0 属于**历史调度记录**（3080 prefill + 395 decode 的双端分工），与最新“3080 全计算 + 395 KV-only”方案分开。
...
- 3080 因远端 KV 通信损耗，牺牲一部分独立运行时的 Prefill / Decode 速度，换来低显存卡原本无法获得的每流 1M 长上下文。
...
- 真实 C1：Prefill **1090 tok/s**、单流 Decode **63.2 tok/s**；C2–C6 聚合 Decode 随并发线性增长，C6 最高 **173.6 tok/s**。
- 适合低并发对话、代码生成等即时交互场景。
...
| KV 精度 | q4_0 | fp8_e4m3 |
| Prefill（Prefill 优先） | **1194.4–1210.6 tok/s** | 1170 / 800 / 615 tok/s（100K / 200K / 300K token 冷清空） |
| Prefill（Decode 优先，C1） | **1090 tok/s** | 未公布 |
| 单流 Decode（C1） | **63.2 tok/s** | 未公布 |
| 聚合 Decode（C6） | **173.6 tok/s** | 未公布 |
| 上下文容量 | **每流 1M** | 1M 配置档 |
| 并发 | C1–C6 全档已测 | 未公布 |
| 拓扑 | 双机异构 PD（3080 全计算 + 395 KV-only 远端存储） | 单机整机 |
...
- **Prefill 优先**路线完整公开 C1–C6 的 Prefill、单流与聚合 Decode；**Decode 优先**路线公开 C1 的 1090 / 63.2 tok/s 与 C6 的 173.6 tok/s 聚合 Decode。
