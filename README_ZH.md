...
**四台 DGX Spark 加两张 RTX 6000Dpro，Prefill 峰值 16698.30 tok/s；按实验者 2026-09-18 校正，展示条件为 32768 输入 / 128 输出 / C12，12/12 请求完成。** 现有机器归档中的同数值记录为 C1，C12 原始批次待补充同步。下表单独保留历史 C1 对照；其中计时、首字等待及倍数均属于该归档批次。

| 历史单请求对照（归档 C1）：32768 输入 / 128 输出 | 纯四 Spark TP4 | 双 6000D＋四 Spark | 比值 |
| --- | ---: | ---: | ---: |
