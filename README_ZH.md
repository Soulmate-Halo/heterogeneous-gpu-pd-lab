...
当前版本：**v2.5 — 稠密加速（异步分层 PD）+ Qwen3.8-27B 双机异构**。公开范围仅包含架构、实测数据、思维演进、结论与限制；不提供部署步骤、复现命令、补丁、端点或内部切层策略。
...
- **v2.1–v2.4** 演进为异步微批流水，让两台设备共同处理一次 prefill；最终形态正式命名为**稠密加速**。
- **v2.4 达到 2129.69 tok/s**：在本地 `9B Q6_K / pp5064` 测试中，比 RTX 3060 基线高 34.0%，比 AI Max+ 395 基线高 119.6%。
...
## Qwen3.8-27B 双机异构实验区 — 独立口径

本节把实验从 RTX 3060 迁移到 RTX 3080 加速头，记录 27B Q4 双机异构 PD 的全量结果。它与上方 `9B Q6_K / pp5064` 主表、以及 `27B IQ3` 验证行均严格分开，不参与 v2.4 百分比计算。
...
**服务态 12 档压测（split vs 395 solo，`bench_3080_cuda_full.log`，ub1024）**
...
TTFT 全档约 **4.5 倍**领先于 solo，prefill 不随并发衰减（稳定 1000–1015 tok/s），decode 无损，KV 迁移税 71 ms。此前 RPC 逐 ubatch 同步带来的 45% 税已消除：去掉 RPC 后同口径从 683.2 提升到 **1000.6 tok/s（+46%）**，约达 3080 裸算 1228 的 82%。
...
详见 [Qwen3.8-27B 双机异构实验记录](results/qwen3.8-27b-dual-machine-pd.zh-CN.md) 与 [机器可读本地数据](data/qwen27b-local-results.csv)。
...
详细记录：[v1.0 独立 PD](results/v1.0-independent-pd.zh-CN.md)、[v2.4 稠密加速演进](results/v2.4-fused-layer-pipeline.zh-CN.md)、[本地 CSV](data/benchmark-results.csv) 与 [更新记录](CHANGELOG_ZH.md)。
