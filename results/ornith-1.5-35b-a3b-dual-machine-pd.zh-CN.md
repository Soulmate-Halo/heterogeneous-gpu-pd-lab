...
100K 表保留 **395 解码段聚合**（C1–C6：23.33–148.20 tok/s），该指标只在 395 解码窗口内测得。整档墙钟派生的 Decode 序列不再展示，原始字段仍保留在 CSV 中用于追溯。

短任务档（1000 in / 128 out）只公布 Prefill 聚合，因为没有单独记录 395 纯后端解码速率。
...
100K prefill 在 C1–C6 间保持约 3.6% 的波动（2895.53 降至 2793.24 tok/s）；395 解码段聚合随并发近似线性增长，从 23.33 增至 148.20 tok/s。
