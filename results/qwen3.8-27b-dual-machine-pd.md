...
| 27B-B | RTX 3080 20GB + AI Max+ 395 | Q4_K_M, serving v1.0 | Remove per-ubatch RPC tax | C1 prefill 1000.6 tok/s; TTFT 1073 ms |
| 27B-C | Same hardware, router v1.1 | Q4_K_M, about 1000 in / 128 out | Serialized prefill plus aggregate decode | 1194.4–1210.6 tok/s prefill; 35–38.5 tok/s repetitive-text single stream |
| 27B-D | 3080 DFlash2 head + 395, router v1.2 | Q4_K_M, random-word seed | Two-stage preemption and KV eviction | C1 single stream 63.2 tok/s; C6 end-to-end aggregate 45.7 tok/s |
...
- Host: AI Max+ 395 / Radeon 8060S, Vulkan decode, full KV ownership, DFlash2 enabled.
...
Envelope: about 1000 prompt tokens / 128 output tokens; router traffic goes through the scheduler and solo traffic connects directly to the 395. Port 8082 serializes prefill, so single-request prefill is also service aggregate prefill. Solo aggregate prefill was not measured and is explicit `NA` in the CSV.
...
Envelope: 1000 in / 128 out with a random-word seed. Router and solo are comparable within this table only; the seed differs from v1.1.
...
## 395 DFlash2 natural-language audit

To test whether “single-stream 35” represents service text, natural-language prose was sent directly to 8081 (395 + DFlash2, `n_max 7`) with chat template, thinking disabled, 128 output tokens, and temperature 0:
...
- The v1.2 3080 has one slot. At C≥2, new prefill preempts the fast decode lane, so the system cannot claim one permanently fast 3080 decode stream.
- Removing the 395 head, changing `n_max 7→3`, and trying 3080 `np2 / -c 8192` remain unmeasured candidates, not completed results.
